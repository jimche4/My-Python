...
path_src = 'C:/PHI/Projects/Prime/Refresh 2023q1/Client Data'
file_orig = 'Prime_converted_4800_ABBR_20190401_20230331.txt'
...
tables that are then used to populate the Excel DQR report for Prime.
...
# Function to truncate tables from different databases
def truncate_table(connection_str, table):
    ...
    print(f'{table} from {connection_str} has been truncated and contains {num_rows:,g} records.')
    conn.close()
    print(f'Connection to {connection_str} has been closed.')


# Table Truncation
print('Created a list of Access table names for truncation.')

# Truncate tables from Prime_disch_px.accdb
print('Truncating tables from 4800_disch_px.accdb\n')
connection_str_disch_px = f"Driver={{Microsoft Access Driver (*.mdb, *.accdb)}};DBQ={os.path.join(path_db, '4800_disch_px.accdb')}"
db_tables_disch_px = ['DISCH_TEMP', 'PX_TEMP', 'PX_TEMP_AGG', 'StdAttributes']
for table in db_tables_disch_px:
    truncate_table(connection_str_disch_px, table)

# Truncate tables from Prime_dx.accdb
print('\nTruncating tables from 4800_dx.accdb\n')
connection_str_dx = f"Driver={{Microsoft Access Driver (*.mdb, *.accdb)}};DBQ={os.path.join(path_db, '4800_dx.accdb')}"
db_tables_dx = ['DX_TEMP', 'DX_TEMP_AGG']
for table in db_tables_dx:
    truncate_table(connection_str_dx, table)

# Truncate tables from Prime_4800.accdb
print('\nTruncating tables from 4800.accdb\n')
connection_str_4800 = f"Driver={{Microsoft Access Driver (*.mdb, *.accdb)}};DBQ={os.path.join(path_db, '4800.accdb')}"
db_tables_4800 = ['DX_TEMP_POA_YR_QTR', 'DX_PX_TEMP_SUMMARY', 'DATE_RANGES',  'R_PHY', 'R_PHY_SUMMARY',]
for table in db_tables_4800:
    truncate_table(connection_str_4800, table)
    

# Function to compact and repair the database
...
# List of Access database filenames
database_files = ['4800_disch_px.accdb', '4800_dx.accdb', '4800.accdb']

# Iterate through the databases and compact each one
print()
print('Compacting the databases after truncation.')
for db_file in database_files:
    database_path = os.path.join(path_db, db_file)
    compact_and_repair(database_path)
    print(f'{db_file} has been compacted successfully.')
print()
...
print('Remove records where PROVNUUM = 000000.')
print(f'{df4800.shape[0]:,g} records are in df4800.', '', sep='\n')
count = df4800[df4800['PROVNUM'] == '000000'].shape[0]
print(f"Number of records where PROVNUM is '000000': {count}")
print('Deleting records where PROVNUM = 000000.')
df4800 = df4800[df4800['PROVNUM'] != '000000']
print(f'{df4800.shape[0]:,g} records remain in df4800.', '', sep='\n')
...
# Function to print data distribution for a field
def print_field_distribution(df, field):
    print(f'The {field} record distribution count is:')
    print(df[field].value_counts(dropna=False), '', sep='\n')
    print(f'The {field} record distribution % is:')
    print(df[field].value_counts(normalize=True, dropna=False), '', sep='\n')

#  now print record counts and distributions by value for each field
for field in FieldToPrint:
    print_field_distribution(df4800, field)
    print()
...
# Function to merge reference data to df4800
def merge_reference_data(df, ref_df, column, field, ref_name):
    df = pd.merge(df, ref_df, on=column, how='left', indicator=True)
    print(f'Added the {field} to df4800 by merging to {ref_name}')
    print(f'via a left join on {column}')
    print(df['_merge'].value_counts(), '', sep='\n')
    df.drop('_merge', axis=1, inplace=True)
    return df

# Now add patient attribute titles to df4800 using merge_reference_data 
for ref_df, column, field, ref_name in ref_data:
    df4800 = merge_reference_data(df4800, ref_df, column, field, ref_name)
...
date_columns = ['ADMDATE', 'DISDATE', 'DOB']
dfDisch[date_columns] = dfDisch[date_columns].apply(pd.to_datetime, format='%m%d%Y')
...
print('Creating additional fields needed for 4800 DQR checks.','',sep='\n')
dfDisch['TOTALCLM'] = dfDisch['TOTALCLM'].fillna(0).astype(float)
dfDisch['Discharges'] = 1
dfDisch['Cases_w_PDX'] = dfDisch['PDX'].notnull().astype(int)
dfDisch['Cases_w_PPX'] = dfDisch['PPX'].notnull().astype(int)

num_nulls = dfDisch['ADMDATE'].isnull().sum()
print(f'The number of null values in date_column is {num_nulls}')

# create LOS column and handle zero values
...
# Print the count
print(f'There are {LOS_zero} records with a zero LOS.','',sep='\n')
...
# Print the count
print('changed records witha nzero LOS to a LOS of 1.')
print(f'There are now {LOS_zero1} records with a zero LOS.','',sep='\n')

#create age columns
dfDisch['AGE_IN_DAYS'] = (dfDisch['ADMDATE'] - dfDisch['DOB']).dt.days
dfDisch['AGE_IN_YEARS'] = dfDisch['AGE_IN_DAYS']/365.25
...
# replace blank values in TOTCLM
...
print(f'The total records expected in dfDx = {df4800.shape[0]*41:,}')
...
conn = pyodbc.connect(connection_str_4800)
cursor = conn.cursor()
